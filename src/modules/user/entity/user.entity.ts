...
export class UserEntity extends BaseEntity {
  ...
  @Column({ name: "avt_img_path", nullable: true, length: 255 })
  avtImgPath: string;

  @Column({ name: "date_of_birth", type: "timestamp", default: () => "CURRENT_TIMESTAMP", nullable: true })
  dateOfBirth: Date;
  ...
  @Column({ name: "validate_otp", nullable: true, length: 10 })
  validateOTP: string;

  @Column({ type: "enum", name: "status", default: statusEnum.ACTIVE, enum: statusEnum })
  status: string;

  @Column({ name: "type_login", default: typeLoginEnum.LOCAL, length: 15 })
  typeLogin: string;

  @Column({ name: "times_validate_otp", default: 3, nullable: false })
